...
class DashboardHtmlTest(unittest.TestCase):
    def test_dashboard_html_contains_core_sections(self) -> None:
        ...
        self.assertIn('if(m.minOi<threshold($("oi-slider"),1e6,1e9,1e6))return false;', html)
        ...
        self.assertIn("function threshold(slider,min,max,zeroValue=0)", html)
        self.assertIn('threshold($("oi-slider"),1e6,1e9,1e6)', html)
        self.assertIn('threshold($("vol-slider"),1e6,1e9)', html)
